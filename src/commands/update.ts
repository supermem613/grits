...
export async function runSelfUpdate(deps: UpdateDeps = {}): Promise<UpdateResult> {
  ...
  if (!isGitRepo(repoRoot)) {
    throw new Error("Grits install directory is not a git repo. Reinstall by cloning the repository, then run npm install and npm link.");
  }
...
function writeHuman(result: UpdateResult): void {
  process.stdout.write("grits repo: " + result.repoRoot + "\n");
...
export async function updateCommand(opts: UpdateOptions = {}): Promise<void> {
  try {
    ...
    if (opts.json) {
      ...
      process.stderr.write("grits update failed: " + hint + "\n");
    }
    process.exitCode = 1;
  }
}
